...
module.exports = async (message) => {
    ...
    if (typeof enable === "undefined") return message.reply(`:x: | Argument boolean is required`);
    if (enable && !channelId) return message.reply(":x: | Argument boolean is truthy, yet channel is not defined");
    if (!validateID(channelId)) return message.reply(":x: | Argument channel is not a valid Discord ID");
    ...
    try {
        ...
        if (!valid) return message.reply(`:x: | Provided channel is not a channel in this guild`);
